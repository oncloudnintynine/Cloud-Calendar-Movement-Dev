// ==========================================
// Admin Settings, User Management & Code Updater
// ==========================================
...
list.innerHTML = tempTypicalEventTypes.map((t, i) => {
  ...
  <div data-idx="${i}" class="flex flex-col sm:flex-row items-start sm:items-center gap-2 bg-white dark:bg-darksurface p-3 rounded-xl border border-gray-300 dark:border-darkborder shadow-sm ${!isFixed ? 'cursor-grab' : ''}">
    ...
    <div class="flex items-center w-full sm:w-auto gap-2 flex-grow">
      <select onchange="updateTypicalEventType(${i}, 'isEvent', this.value === 'true')" class="border-2 border-gray-300 dark:border-gray-600 rounded-lg py-1.5 px-2 bg-gray-50 dark:bg-[#1a1a1a] text-gray-900 dark:text-white outline-none focus:border-blue-500 text-sm cursor-pointer shrink-0">
         <option value="true" ${t.isEvent ? 'selected' : ''}>Time-Bound</option>
         <option value="false" ${!t.isEvent ? 'selected' : ''}>All/Half-Day</option>
      </select>
      
      <input type="text" value="${t.defaultLoc || ''}" onchange="updateTypicalEventType(${i}, 'defaultLoc', this.value)" placeholder="Default Location" class="flex-grow min-w-[100px] border-2 border-gray-300 dark:border-gray-600 rounded-lg py-1.5 px-2 bg-gray-50 dark:bg-[#1a1a1a] focus:bg-white dark:focus:bg-black text-gray-900 dark:text-white outline-none focus:border-blue-500 transition text-sm" ${!t.isEvent ? 'disabled style="opacity:0.5"' : ''}>
...
const locInput = document.getElementById('new-typical-event-loc');
if(nameInput && nameInput.value.trim()) { 
  tempTypicalEventTypes.push({ 
    name: nameInput.value.trim(), 
    isEvent: isEventInput.value === 'true',
    defaultLoc: locInput.value.trim() 
  }); 
  nameInput.value = ''; 
  locInput.value = '';
  renderTypicalEventTypes(); 
...
try {
    ...
    for (const file of fileNodes) {
        ...
        compiledFiles.push({ path: file.path, content: content });
    }
...
async function triggerCodeUpdate() {
const repo = document.getElementById('set-github-repo').value.trim();
const token = document.getElementById('set-github-token').value.trim();
const rawInput = document.getElementById('set-github-code-input').value.trim();

if (!repo || !token || !rawInput) return alert('Please provide the GitHub Repo, a valid Fine-Grained Token, and paste the code output.');
