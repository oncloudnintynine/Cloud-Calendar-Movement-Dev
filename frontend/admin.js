...
function renderMenuOrder() {
  ...
  window.menuSortable = new Sortable(list, {
    ...
    onEnd: function () { tempMenuOrder = Array.from(list.children).map(el => el.dataset.id); }
  });
}
...
function renderLeaveTypes() {
  ...
  window.leaveTypeSortable = new Sortable(list, {
    animation: 150,
    handle: '.handle-leave',
    ghostClass: 'opacity-50',
    onEnd: function () { tempLeaveTypes = Array.from(list.children).map(el => el.dataset.val); }
  });
}
...
function renderKAHSelected() {
  ...
  // Create mapping of Unit -> Parent Unit
  const unitToParent = {};
  for (const parent in companyStructure) {
    companyStructure[parent].forEach(child => { unitToParent[child] = parent; });
  }

  // Group KAH by Parent -> Child
  const grouped = {};
  adminKAHList.forEach(k => {
    const parent = unitToParent[k.dept] || (companyStructure[k.dept] ? k.dept : 'Uncategorized');
    if (!grouped[parent]) grouped[parent] = {};
    if (!grouped[parent][k.dept]) grouped[parent][k.dept] = [];
    grouped[parent][k.dept].push(k);
  });
  ...
  for (const parent in grouped) {
    ...
    for (const child in grouped[parent]) {
      if (child !== parent && child !== 'Uncategorized') {
        html += `<h5 class="font-semibold text-sm text-gray-600 dark:text-gray-400 mt-2 mb-1 pl-2 border-l-2 border-blue-400">${child}</h5>`;
      }
      html += `<div class="space-y-1">`;
      grouped[parent][child].forEach(k => {
        html += `
          <div class="flex justify-between items-center bg-white dark:bg-darksurface p-2 rounded-lg border dark:border-darkborder shadow-sm pl-4">
            <span class="font-medium">${k.name} <span class="text-xs text-gray-500 ml-1">(${k.dept})</span></span>
