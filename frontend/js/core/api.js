...
window.failedSyncRecoveryCache = {}; // Cache to self-heal temporary UI state if server rejects
...
try {
   ...
   // If it's a fatal hard error from the server (e.g. timeout, KAH rules strictly blocked it, unauthorized)
   if (e.message.includes('Unauthorized') || e.message.includes('busy processing') || e.message.includes('fatal')) {
       // Self-heal: revert the optimistic UI modification to match original state
       if (task.action === 'submitLeave') {
           allLeaves = allLeaves.filter(l => l.ID !== task.payload.id); // Erase the fake record
...
// Global Foreground Poller for real-time concurrent updates
...
if (!window.isOffline && window.syncQueue.length === 0) {
   processSyncQueue(); // Implicitly triggers a fetch if queue is 0 inside processSyncQueue logic
}
