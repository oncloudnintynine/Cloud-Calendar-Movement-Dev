...
if(btn) btn.innerHTML = `<svg class="w-5 h-5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z" /><circle cx="12" cy="12" r="3" /></svg> Show Cal`;
} else {
...
let baseClass = "cal-day-cell relative flex items-center justify-center w-8 h-8 lg:w-10 lg:h-10 mx-auto rounded-full cursor-pointer transition-colors text-sm lg:text-base font-medium ";
...
if (cellDay === d) {
baseClass = "cal-day-cell relative flex items-center justify-center w-8 h-8 lg:w-10 lg:h-10 mx-auto rounded-full cursor-pointer transition-colors text-sm lg:text-base font-bold bg-blue-600 text-white shadow-md ";
}
...
let html = '<div class="flex flex-col flex-grow bg-gray-200 dark:bg-darkborder gap-px border border-gray-300 dark:border-darkborder rounded-lg overflow-hidden shadow-inner">';
...
let rowHeight = Math.max(100, (slots.length * 24) + 36);
...
<div class="text-xs lg:text-sm font-bold ${isToday ? 'bg-blue-600 text-white rounded-full w-[26px] h-[26px] lg:w-[30px] lg:h-[30px] mx-auto flex items-center justify-center shadow-md' : 'text-gray-500 dark:text-darkmuted text-center'}">${curD.getDate()}</div>
</div>`;
}

html += `<div class="absolute top-10 lg:top-12 left-0 right-0 bottom-0 pointer-events-none overflow-hidden">`;
segments.forEach(seg => {
...
html += `<div class="absolute h-[22px] px-1.5 text-[11px] lg:text-[12px] font-bold flex items-center truncate shadow-sm pointer-events-auto cursor-pointer border border-black/5 ${color} ${rounded}" style="left: calc(${left}% + 2px); width: calc(${width}% - 4px); top: ${topOffset}px;" onclick="selectDate('${ctx}', ${w.getFullYear()}, ${w.getMonth()}, ${w.getDate() + seg.sDay})" title="${appliedTitle}">${appliedTitle}</div>`;
});
html += `</div></div>`; 
}
html += '</div>';
...
if(leaveType === 'Public Holiday') return 'bg-indigo-100 text-indigo-800 dark:bg-indigo-900/40 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800';
const safeStatus = String(status || '');
if(safeStatus.includes('Pending')) return 'bg-orange-100 text-orange-800 dark:bg-orange-900/40 dark:text-orange-300 border border-orange-200 dark:border-orange-800';
if(safeStatus.includes('Cancelled')) return 'bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-300 border border-red-200 dark:border-red-800';
...
return `Cal Updated<br><span class="text-[10px] lg:text-xs font-bold text-red-600 dark:text-red-400 tracking-tight leading-tight block mt-1 uppercase">KAH Limit Crossed</span><span class="text-[11px] lg:text-sm font-bold text-red-700 dark:text-red-300 tracking-tight leading-none block mt-0.5">${dept}</span>`;
}
return s;
}
...
if (line !== '') {
   validLines.push(`<p class="text-sm lg:text-base text-gray-600 dark:text-darkmuted mt-1 leading-snug">${line}</p>`);
}
}
}
...
if (!items || items.length === 0) return isInfoAllContext ? '' : `<p class="text-gray-500 dark:text-darkmuted text-center italic mt-4 text-base">No records for this date.</p>`;
...
actionBtns = `
<button onclick="triggerEdit('${l.ID}')" class="p-2 text-blue-600 hover:text-blue-800 hover:bg-blue-50 dark:text-blue-400 dark:hover:text-blue-300 dark:hover:bg-blue-900/30 rounded-xl transition shrink-0" title="Edit Record">
<svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z" /><path d="m15 5 4 4" /></svg>
</button>
<button onclick="cancelLeave('${l.ID}', '${l.Phone}')" class="p-2 text-red-600 hover:text-red-800 hover:bg-red-50 dark:text-red-400 dark:hover:text-red-300 dark:hover:bg-red-900/30 rounded-xl transition shrink-0" title="Cancel Record">
<svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg>
</button>`;

compactActionBtns = `
<button onclick="triggerEdit('${l.ID}')" class="p-1.5 text-blue-600 hover:text-blue-800 hover:bg-blue-100 dark:text-blue-400 dark:hover:text-blue-300 dark:hover:bg-blue-900/40 rounded-lg transition shrink-0" title="Edit Record">
...
<button onclick="cancelLeave('${l.ID}', '${l.Phone}')" class="p-1.5 text-red-600 hover:text-red-800 hover:bg-red-100 dark:text-red-400 dark:hover:text-red-300 dark:hover:bg-red-900/40 rounded-lg transition shrink-0" title="Cancel Record">
...
if (isInfoAllContext) {
return `<div class="p-3 rounded-xl border border-blue-200 dark:border-blue-800/60 bg-white/60 dark:bg-black/20 flex flex-col mb-2 last:mb-0">
<div class="flex justify-between items-start ${hasBody ? 'cursor-pointer select-none' : ''}" ${hasBody ? 'onclick="toggleAgendaCard(this)"' : ''}>
<h3 class="font-bold text-sm lg:text-base text-blue-900 dark:text-blue-300 flex-grow pr-3 leading-tight">${finalTitle}</h3>
${hasBody ? `<svg class="w-5 h-5 text-blue-500 transition-transform duration-200 chevron-icon shrink-0 mt-0.5 ${isCollapsed ? '' : 'rotate-180'}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><path d="m6 9 6 6 6-6"/></svg>` : ''}
</div>
${hasBody ? `
...
return `<div class="border border-gray-300 dark:border-darkborder p-4 lg:p-5 rounded-2xl shadow-sm bg-white dark:bg-darksurface flex flex-col transition hover:border-blue-400 dark:hover:border-blue-600">
<div class="flex justify-between items-start ${hasBody ? 'cursor-pointer select-none' : ''}" ${hasBody ? 'onclick="toggleAgendaCard(this)"' : ''}>
<div class="flex-grow pr-3">
<h3 class="font-bold text-base lg:text-lg text-gray-900 dark:text-gray-100 leading-tight">${finalTitle}</h3>
${!isMyCalendar && !isEvent && l.HalfDay !== 'None' && l.HalfDay !== 'NONE' ? `<p class="font-medium text-sm lg:text-base text-gray-700 dark:text-darktext mt-1">(${l.HalfDay})</p>` : ''}
</div>
<div class="flex items-center shrink-0 mt-0.5">
<span class="text-xs lg:text-sm font-bold px-3 py-1.5 rounded-lg text-center inline-block leading-tight shadow-sm ${getBadgeClass(l.Status, l.LeaveType)}">${formatStatusBadge(l.Status, l.LeaveType)}</span>
${hasBody ? `<svg class="w-6 h-6 ml-2 text-gray-400 dark:text-darkmuted transition-transform duration-200 chevron-icon shrink-0 ${isCollapsed ? '' : 'rotate-180'}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><path d="m6 9 6 6 6-6"/></svg>` : ''}
</div>
</div>
${hasBody ? `
<div class="agenda-card-body ${isCollapsed ? 'hidden-view' : ''}">
...
${actionBtns ? `<div class="flex shrink-0 space-x-1.5 pb-1">${actionBtns}</div>` : ''}
</div>
</div>` : ''}
</div>`;
}).join('');
...
<div class="agenda-day-group mb-8" data-date="${yyyy}-${mm}-${dd}">
   <div class="sticky top-0 bg-gray-50/95 dark:bg-[#1a1a1a]/95 backdrop-blur-sm z-10 py-2.5 border-y border-gray-200 dark:border-darkborder mb-4 shadow-sm px-4 rounded-xl">
       <h3 class="font-bold text-base lg:text-lg text-blue-700 dark:text-blue-400 tracking-wide">${formatDisplayDate(d)}</h3>
   </div>
   <div class="space-y-4 px-1 lg:px-2">
...
container.innerHTML = html || `<p class="text-gray-500 dark:text-darkmuted text-center mt-10 text-lg">No records found.</p>`;
...
group.innerHTML = `
<div class="sticky top-0 bg-gray-50/95 dark:bg-[#1a1a1a]/95 backdrop-blur-sm z-10 py-2.5 border-y border-gray-200 dark:border-darkborder mb-4 shadow-sm px-4 rounded-xl">
   <h3 class="font-bold text-base lg:text-lg text-blue-700 dark:text-blue-400 tracking-wide">${formatDisplayDate(targetDateObj)}</h3>
</div>
<div class="space-y-4 px-1 lg:px-2">
   <p class="text-gray-500 dark:text-darkmuted text-center italic mt-4 text-base">No records for this date.</p>
</div>`;
