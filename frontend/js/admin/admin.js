...
if (!confirm("This will scan the selected Google Calendars for new external events. Proceed?")) return;
showLoader(true);
try {
await apiCall('forceSyncExternalCals', { adminPass: user.pass });
await loadLeavesData(); // Re-fetch globally to update UI seamlessly
alert("Sync completed successfully. Dashboard updated.");
} catch(e) {
...
roleHtml = `<span class="px-2.5 py-1 rounded

Apologies for the cutoff. I bundled too many files into the last batch. Here is the complete, untruncated `frontend/js/admin/admin.js` by itself.

### Batch 2 Delivery

@@@===FILE_PATH: frontend/js/admin/admin.js ===@@@
@@@===CODE_START===@@@
// ==========================================
// Admin Settings, User Management & Calendar Access
// ==========================================
